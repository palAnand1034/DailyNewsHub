...
def news_list(request):
    ...
    # Master user Access Start
    perm = 0
    for group in request.user.groups.all():
        if group.name == "masteruser":
            perm = 1
            break  # Exit the loop if the user is a master user

    # Fetch news based on user permissions
    if perm == 0:
        news_queryset = News.objects.filter(writer=request.user)
    else:
        news_queryset = News.objects.all()
    # Master user Access End

    # Pagination Start
    paginator = Paginator(news_queryset, 7)  # Number of news items per page
    page = request.GET.get('page')

    try:
        news = paginator.page(page)
    except EmptyPage:
        news = paginator.page(paginator.num_pages)  # Get the last page
    except PageNotAnInteger:
        news = paginator.page(1)  # Default to the first page
    # Pagination End
